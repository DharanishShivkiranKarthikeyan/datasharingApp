...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (isIndexPage) {
    if (!signupButton || !loginButton || !logoutButton || !userBalanceElement || !publishButton || !searchButton || !depositButton || !withdrawButton || !toggleHistoryButton || !transactionHistory || !publishedItemsTableBody || !buyHashButton || !createGroupButton || !chatContacts || !chatWindow || !chatInput || !sendChatButton || !chatList) {
...
async function loadGroupChats() {
  ...
  groupChatsSnapshot.forEach(doc => {
    ...
    li.className = 'p-2 bg-gray-600 rounded cursor-pointer hover:bg-gray-500';
    li.innerHTML = `
      Group: ${group.name} (${group.participants.length} members)
      <button onclick="window.inviteToGroup('${doc.id}')" class="ml-2 bg-green-500 text-white rounded px-2 py-1">Invite</button>
      <button onclick="window.leaveGroup('${doc.id}')" class="ml-2 bg-red-500 text-white rounded px-2 py-1">Leave</button>
    `;
    li.onclick = (e) => {
      if (e.target.tagName !== 'BUTTON') {
        startGroupChat(doc.id, group.name, group.participants, group.participantUsernames);
      }
    };
...
async function inviteToGroup(groupId) {
  ...
  const userDirectory = window.userDirectory || await fetchUserDirectory();
  const userToInvite = userDirectory.find(u => u.username === username);
  if (!userToInvite) {
  ...
  showToast(`Invited ${username} to the group!`);
  loadGroupChats();
}
